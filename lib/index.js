...
function Property( key, options ) {
  ...
  // Placeholder for custom casting methods
  var _customCast;

  // Derive a caster function based on type (undefined if none)
  Object.defineProperty( this, 'caster', {
    enumerable: true,
    get: function () { return _customCast
      ? _customCast
      // @todo get `options` out of here - create copies of required info only
      : _getCaster( options ); },
    set: function (fn) { _customCast = fn; }
  });
...
Property.prototype.cast = function( val ) {
  ...
  // Cast by function, if present
  if (this.caster) return this.caster( val, this.type );

  // Otherwise check if this is a schema
  if (this.type && this.type.toRecord) return this.type.toRecord( val );
