...
/**
  Helper method: Applies a default and runs transforms on a `val`

  @param {Mixed} val The value to cast
  @param {Schema} schema The schema to apply to the value

  @throws {Error} on failed transform
  ...
  try {
    ...
    // Cast everything else
      ...
      return schema.schema
        ...
        : transform( setDefault( val, schema ), schema.transforms );
    }
  }
  catch (e) {
    var err = [e.message];
...
/**
  Casts (default+transform) arrays and objects that are described by sub-schema
